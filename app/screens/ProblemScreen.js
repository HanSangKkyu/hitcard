...
export default function ProblemScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={{  }}>
      <View style={{ margin: 20, borderBottomWidth: 1, flexDirection: 'row', paddingBottom: 10, }}>
        <TouchableOpacity style={{ marginRight: 20, alignSelf: 'center' }} onPress={() => navigation.goBack()} >
          ...
          {isSearch ? null : <Text style={{ flex: 1, fontSize: 22, alignSelf: 'center' }}>~ 카테고리 수정</Text>}
      ...
      <View style={{flexDirection:'row', position:'absolute', bottom:0, left:0, backgroundColor:'white', width:WINDOW_WIDTH, }}>
        <TouchableOpacity onPress={()=>{navigation.navigate('SolveScreen')}} style={{flex:1, alignSelf:'flex-start'}}>
          <Text style={{alignSelf:'center', fontSize:30}}>다른 카테고리로 이동</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={()=>{navigation.navigate('SolveScreen')}} style={{flex:1, alignSelf:'flex-end'}}>
          <Text style={{alignSelf:'center', fontSize:30}}>삭제</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
